...
export class WebGlRenderer extends AbstractCanvasRenderer {
    ...
    public drawTileMap(tileMap: TileMap): void {
        ...
        tileMapDrawer.setUniform(this.simpleRectDrawer.u_vertexMatrix,pos16h.mat16);
        pos16h.release();
        rect.release();
        size.release();

        tileMapDrawer.setUniform(this.simpleRectDrawer.u_textureMatrix,IDENTITY);
        tileMapDrawer.attachTexture('texture',tileMap.spriteSheet.getResourceLink().getTarget() as Texture);
        tileMapDrawer.draw();
    }
