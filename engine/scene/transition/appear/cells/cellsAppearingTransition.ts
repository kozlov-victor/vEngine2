...
export abstract class AbstractCellsAppearingTransition extends AbstractSceneTransition {
    ...
    constructor(
        ...
        protected numOfCellsX:number = 10,
        protected numOfCellsY:number = 10,
    ...
    protected renderScenes(a:Optional<Scene>,b:Optional<Scene>): void {
        ...
        const cellWidth:number = this.game.getCurrScene().size.width/this.numOfCellsX;
        const cellHeight:number = this.game.getCurrScene().size.height/this.numOfCellsY;
        ...
        if (a!==undefined) {
            ...
            for (let y:number = 0; y < this.numOfCellsY; y++) {
                for (let x:number = 0; x < this.numOfCellsX; x++) {
                    progress++;
                    const currProgressRelative = progress/total*100;
                    if (currProgressRelative<this.val) {
                        this.lockingRect.setXYWH(x*cellWidth,y*cellHeight,cellWidth,cellHeight);
