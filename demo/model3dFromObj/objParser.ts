...
export class ObjParser {
    ...
    private readPoint3(s:string):Point3{
        ...
        s.split(' ').filter(it=>it.trim().length).map(it=>parseFloat(it)).forEach((n:number,i:number)=>{
            if (i===0) point3.x = n;
    ...
    private readFace(s:string):Face{
        ...
        s.split(' ').filter(it=>it.trim().length).forEach((triplet:string,i:number)=>{
    ...
    private readObj(source:string):obj{
        ...
        source.split('\n').forEach((line:string)=>{
            ...
            switch (command) {
                case 'vn':
                    obj.vn_arr.push(this.readPoint3(restSymbols));
                    break;
                case 'vt':
                    obj.vt_arr.push(this.readPoint2(restSymbols));
                    break;
                case 'v':
                    obj.v_arr.push(this.readPoint3(restSymbols));
                    break;
                case 'f':
                    obj.f_arr.push(this.readFace(restSymbols));
                    break;
            }
        });
        console.log(obj);
    ...
    public objToPrimitive(obj:obj):AbstractPrimitive{
        const pr:ObjPrimitive = new ObjPrimitive();
        let cnt:number = 0;
        obj.f_arr.forEach((f:Face,i:number)=>{

            const vert1:Point3 = obj.v_arr[f[0].v-1];
            const vert2:Point3 = obj.v_arr[f[1].v-1];
            const vert3:Point3 = obj.v_arr[f[2].v-1];

            let norm1:Point3 = obj.vn_arr[f[0].n-1];
            let norm2:Point3 = obj.vn_arr[f[1].n-1];
            let norm3:Point3 = obj.vn_arr[f[2].n-1];
            ...
            const tex1:Point2 = obj.vt_arr[f[0].uv-1];
            const tex2:Point2 = obj.vt_arr[f[1].uv-1];
            const tex3:Point2 = obj.vt_arr[f[2].uv-1];
