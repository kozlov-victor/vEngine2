...
export class ControlPointByLengthPassedResolver {
    ...
    private _currentControlPointIndex = 0;
    private _controlPoints:IControlPoint[] = [];
    private _totalLength = 0;
    private point = new PointWithAngle();

    constructor(polyLine:PolyLine) {
        const zeroPoint = new Point2d();
        polyLine.getSegments().forEach((c:Readonly<Line>)=>{
            const length = MathEx.getDistance(zeroPoint,c.pointTo);
    ...
    private getCurrentControlPoint(lengthPassed:number):Optional<IControlPoint> {
        ...
        let result = this._currentControlPointIndex;
    ...
    public nextPointByLengthPassed(lengthPassed:number):Optional<IPointAndAngle> {
        const point = this.getCurrentControlPoint(lengthPassed);
        if (point===undefined) return undefined;
        const lengthPassedRelative = lengthPassed - point.from;
        const passedFactor =
            point.length===0? 0: lengthPassedRelative/point.length;
        const x = point.pointFrom.x + (point.pointTo.x - point.pointFrom.x)*passedFactor;
        const y = point.pointFrom.y + (point.pointTo.y - point.pointFrom.y)*passedFactor;
...
export class MoveByPathAnimation extends AbstractMoveAnimation {
    ...
    protected override onUpdate():void{
        ...
        const lengthPassed = this.velocity * this.passedTime / 1000;
        const point = this._controlPointResolver.nextPointByLengthPassed(lengthPassed);
